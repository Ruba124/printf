	...
	int flags, int width, int precision, int size)
{
	...
	if ((flags & N_ZERO) && !(flags & N_MINUS))
		padd = '0';
	if (is_negative)
		extra_ch = '-';
	else if (flags & N_PLUS)
		extra_ch = '+';
	else if (flags & N_SPACE)
		extra_ch = ' ';
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (j = 1; j < width - length + 1; j++)
			buffer[j] = padd;
		buffer[j] = '\0';
		if (flags & N_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[1], j - 1));
		}
		else if (!(flags & N_MINUS) && padd == ' ')/* extra char to left of buff */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], j - 1) + write(1, &buffer[ind], length));
		}
		else if (!(flags & N_MINUS) && padd == '0')/* extra char to left of padd */
		{
			if (extra_c)
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & N_MINUS) /* Asign extra char to left of buffer [buffer>padd]*/
		{
